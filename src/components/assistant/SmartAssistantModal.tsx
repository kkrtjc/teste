...
export const SmartAssistantModal = memo(function SmartAssistantModal({
  ...
  // Monitora a posição exata do elemento alvo destacado
  useEffect(() => {
    ...
    const updateTarget = () => {
      ...
      if (isMobile) {
        // Tenta ID específico móvel primeiro se houver
      ...
      if (!el) {
        el = document.querySelector(`#${targetId}`) || document.querySelector(`[data-guide-id="${targetId}"]`);
      }

      if (el) {
        const rect = el.getBoundingClientRect();
        // Só define se o elemento tiver dimensões visíveis
    ...
    updateTarget();
    const interval = setInterval(updateTarget, 120);
  ...
  const getFluidCardStyle = (): React.CSSProperties => {
    if (!targetRect) {
      return isMobile 
        ? { position: 'fixed', bottom: '90px', left: '12px', right: '12px', zIndex: 10001 }
    ...
    if (isMobile) {
      if (targetCenterY > screenHeight * 0.45) {
        // Elemento destacado está na metade inferior da tela -> Card fica no topo!
        return {
          position: 'fixed',
          top: '64px',
          left: '12px',
        ...
        // Elemento destacado está na metade superior da tela -> Card fica abaixo!
        return {
          position: 'fixed',
          bottom: '90px',
          left: '12px',
    ...
    if (targetCenterX < 280) {
      // Elemento está na barra lateral esquerda -> Posiciona à direita dele
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] pointer-events-auto">
      {/* ── Spotlight com Borda Pulsante Elegante & Recorte Perfeito ── */}
      {targetRect ? (
        <div
          onClick={onClose}
          className="fixed z-[10000] border-2 border-amber-400 rounded-2xl pointer-events-auto cursor-pointer transition-all duration-300 assistant-spotlight-pulse"
          style={{
          ...
          title="Clique para fechar apresentação"
        />
      ) : (
        <div 
          onClick={onClose}
          className="absolute inset-0 bg-slate-950/70 backdrop-blur-[2px] transition-opacity duration-300"
        />
      )}

      {/* Top Banner de Contexto de IA */}
      <div className="fixed top-3 left-1/2 -translate-x-1/2 z-[10001] bg-slate-900/95 border border-amber-500/50 px-4 py-1.5 rounded-full shadow-[0_10px_25px_rgba(0,0,0,0.8)] backdrop-blur-md flex items-center gap-2 pointer-events-none animate-bounce-subtle">
      ...
      <div 
        ...
        className="transition-all duration-400 ease-out animate-scale-up z-[10001]"
      >
        <div className="bg-[#0f111a]/95 border-2 border-amber-500/40 rounded-3xl p-5 shadow-[0_25px_60px_rgba(0,0,0,0.95)] backdrop-blur-xl relative overflow-hidden flex flex-col gap-3.5">
          ...
          <div className="flex items-center justify-between border-b border-white/10 pb-3">
            ...
            <div className="flex items-center gap-1">
              ...
              {isSpeaking && (
                <div className="flex items-center gap-0.5 px-2 py-1 bg-amber-500/10 border border-amber-500/20 rounded-lg mr-1" title="Mura IA falando">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-zinc-300 leading-relaxed font-normal bg-black/30 p-3 rounded-2xl border border-white/5">
