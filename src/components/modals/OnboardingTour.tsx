...
const TOUR_STEPS: TourStep[] = [
  {
    ...
    visualSnippet: (
      <div className="bg-gradient-to-r from-amber-500/20 via-orange-500/10 to-amber-500/20 border border-amber-500/30 rounded-2xl p-4 text-center space-y-2 my-2">
        <div className="flex items-center justify-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-amber-400 text-black font-black flex items-center justify-center text-sm shadow-md">M</div>
          <span className="font-black text-white text-base tracking-tight">MURA<span className="text-amber-400">MANAGER</span></span>
        </div>
    ...
    visualSnippet: (
      <div className="grid grid-cols-3 gap-2 my-2">
        <div className="bg-white/5 border border-white/10 p-2.5 rounded-xl text-center">
          ...
          <p className="text-sm font-black text-amber-400">Ativas</p>
        </div>
        <div className="bg-white/5 border border-white/10 p-2.5 rounded-xl text-center">
          <p className="text-[9px] text-white/50 font-bold uppercase">Postura</p>
          <p className="text-sm font-black text-green-400">Diária</p>
        </div>
        <div className="bg-white/5 border border-white/10 p-2.5 rounded-xl text-center">
          <p className="text-[9px] text-white/50 font-bold uppercase">Lotes</p>
          <p className="text-sm font-black text-blue-400">100% OK</p>
        </div>
      </div>
    )
  },
  {
    ...
    visualSnippet: (
      <div className="bg-black/40 border border-amber-400/30 rounded-2xl p-3 my-2 text-center">
        <p className="text-[10px] text-amber-400 font-bold uppercase tracking-wider mb-1">🌳 Árvore Genealógica Integrada</p>
        <div className="flex justify-center gap-2 items-center text-[10px] text-white/70">
          <span className="px-2 py-0.5 rounded bg-white/10">Bisavós</span> →
          <span className="px-2 py-0.5 rounded bg-white/10">Avós</span> →
          <span className="px-2 py-0.5 rounded bg-amber-400/20 text-amber-300 font-bold">Ave</span>
        </div>
      </div>
    )
  },
  {
    ...
    visualSnippet: (
      <div className="flex gap-2 my-2">
        <div className="flex-1 bg-amber-500/10 border border-amber-500/30 p-2 rounded-xl text-center">
          <p className="text-[10px] text-amber-300 font-bold">🥚 Postura</p>
        </div>
        <div className="flex-1 bg-blue-500/10 border border-blue-500/30 p-2 rounded-xl text-center">
          <p className="text-[10px] text-blue-300 font-bold">🐣 Pintinhos</p>
        </div>
        <div className="flex-1 bg-orange-500/10 border border-orange-500/30 p-2 rounded-xl text-center">
  ...
  },
  {
    ...
    highlights: [
      ...
      'Registo de casais e reprodutores de destaque',
      'Histórico de progênie e linhagens pura Mura'
    ],
    visualSnippet: (
      <div className="bg-gradient-to-r from-purple-500/20 to-blue-500/20 border border-purple-500/30 rounded-2xl p-3 my-2 text-center">
...
export function OnboardingTour({
  ...
  // Reset to first step and scroll to top when tour opens
  useEffect(() => {
    if (isOpen) {
  ...
  // Navigate to step path when step changes and scroll top
  useEffect(() => {
    if (isOpen && step?.path) {
      navigate(step.path);
      window.scrollTo({ top: 0, behavior: 'smooth' });
  ...
  return createPortal(
    <div className="fixed inset-0 z-[250] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/60 backdrop-blur-sm animate-fade-in select-none">
      
      {/* Top Floating Spotlight Badge Pointer */}
      <div className="fixed top-3 left-1/2 -translate-x-1/2 z-[260] hidden sm:flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#0F172A]/90 border border-amber-400/50 shadow-[0_0_20px_rgba(245,158,11,0.3)] animate-bounce-x">
      ...
      {/* Container Principal do Card do Tutorial */}
      <div className="bg-[#0F172A]/95 border-t sm:border border-amber-400/40 w-full sm:max-w-lg rounded-t-3xl sm:rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.9),0_0_25px_rgba(245,158,11,0.25)] overflow-hidden flex flex-col animate-scale-up relative max-h-[85dvh]">
        
        {/* Top Header Bar */}
        <div className="px-6 pt-5 pb-3 border-b border-white/10 flex items-center justify-between bg-white/[0.03]">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-white/50 font-bold">Passo {currentStepIndex + 1} de {TOUR_STEPS.length}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Content Body */}
        <div className="p-6 space-y-4 overflow-y-auto flex-1" key={step.id}>
          <div>
        ...
        <div className="p-4 border-t border-white/10 bg-black/40 flex items-center justify-between gap-3">
          {!isFirst ? (
            <button
              onClick={handlePrev}
              className="py-2.5 px-4 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-white text-xs font-bold transition-all flex items-center gap-1.5 active:scale-95"
            >
          ...
          <button
            onClick={handleNext}
            className="py-2.5 px-6 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-black font-black text-xs transition-all shadow-[0_0_20px_rgba(245,158,11,0.35)] active:scale-95 flex items-center gap-2"
          >
            {isLast ? (
              <>Concluir Manual <Sparkles size={16}/></>
            ) : (
