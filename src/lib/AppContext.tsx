...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [isTourOpen, setIsTourOpen] = useState<boolean>(() => {
    return localStorage.getItem('@mura-manager:hasSeenTour_v15.0') !== 'true';
  });
  ...
  const closeTour = () => {
    setIsTourOpen(false);
    localStorage.setItem('@mura-manager:hasSeenTour_v15.0', 'true');
    if (localStorage.getItem('@mura-manager:hasSetupProfile_v15.0') !== 'true') {
  ...
  const finishProfileSetup = () => {
    ...
    localStorage.setItem('@mura-manager:hasSetupProfile_v15.0', 'true');
  };
